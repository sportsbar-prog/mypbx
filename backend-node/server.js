...
async function generatePJSIPConfigForTrunk(trunk, deleteTrunkName = null) {
  try {
    ...
    let config = '';
    try {
      config = fs.readFileSync(PJSIP_TRUNKS_CONF, 'utf8');
    } catch (e) {
      console.log('Creating new pjsip_trunks.conf');
      config = '; PJSIP Trunks - Generated by Asterisk GUI\n\n';
    }
...
`;
    ...
    // Write back to file
    fs.writeFileSync(PJSIP_TRUNKS_CONF, config, 'utf8');
