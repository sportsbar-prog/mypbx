...
function ensurePjsipIncludes() {
  ...
  // Check if config has includes
  const hasTrunksInclude = config.match(/#include\s+pjsip_trunks\.conf/);
  const hasUsersInclude = config.match(/#include\s+pjsip_users\.conf/);
  
  // If config is empty or doesn't have proper structure, rewrite it
  if (!config || !hasTrunksInclude || !hasUsersInclude) {
    const cleanConfig = `; PJSIP Configuration
; Auto-generated by Asterisk GUI
...
; Include trunk and user configurations
#include pjsip_trunks.conf
#include pjsip_users.conf
`;
    
    fs.writeFileSync(PJSIP_CONF, cleanConfig, 'utf8');
    console.log('✅ Regenerated clean pjsip.conf with includes');
  }
}
...
async function readAsteriskConfig(filename) {
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      console.log(`ℹ️  File not found, creating default: ${safeName}`);
      const defaultContent = getDefaultConfig(safeName);
      if (defaultContent) {
        const writeResult = await writeAsteriskConfig(safeName, defaultContent);
        if (writeResult.success) {
          return { success: true, content: defaultContent, filename: safeName, path: filePath, created: true };
        }
      }
    }
...
; Endpoint template for basic settings
[endpoint-template](!)
type=endpoint
dtmf_mode=rfc4733
disallow=all
...
; Include trunks and user configurations
#include pjsip_trunks.conf
#include pjsip_users.conf
`;
  }
  
  if (filename === 'pjsip_trunks.conf') {
    return `; PJSIP Trunks Configuration
; Auto-generated by ARI API
; Add SIP trunks via the Trunk Management page

`;
  }
  
  if (filename === 'pjsip_users.conf') {
    return `; PJSIP Users Configuration
; Auto-generated by ARI API
; Add SIP endpoints via the SIP Users page
...
[internal]
exten => _XXX,1,NoOp(Internal extension \${EXTEN})
same => n,Dial(PJSIP/\${EXTEN},30)
...
app.post('/api/asterisk/sip-users/apply', authenticateAdmin, async (req, res) => {
  try {
    ...
    console.error('Error applying SIP users:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
// In-memory queue configuration (persisted to queues.conf)
let queues = [];

const QUEUES_CONF = path.join(ASTERISK_CONFIG_DIR, 'queues.conf');

// Parse a minimal queues.conf (sections with key=value)
function parseQueuesConf(content) {
  const result = [];
  let current = null;
  const lines = content.split(/\r?\n/);
  for (const raw of lines) {
    const line = raw.trim();
    if (!line || line.startsWith(';') || line.startsWith('[') && line.includes('general')) {
      continue;
    }
    const section = line.match(/^\[(.+)\]$/);
    if (section) {
      if (current) result.push(current);
      current = { name: section[1], members: [] };
      continue;
    }
    const kv = line.match(/^([^=]+)=(.*)$/);
    if (kv && current) {
      const key = kv[1].trim();
      const val = kv[2].trim();
      if (key === 'member' || key === 'member_interface') {
        current.members.push({ interface: val, penalty: 0 });
      } else {
        current[key] = val;
      }
    }
...
// Serialize queues to queues.conf
function serializeQueuesConf(items) {
  let out = '; queues.conf - generated by Asterisk GUI\n\n';
  out += '[general]\n';
  out += 'persistentmembers = yes\n';
  out += 'autofill = yes\n\n';

  for (const q of items) {
    out += `[${q.name}]\n`;
    out += `strategy=${q.strategy || 'ringall'}\n`;
    if (q.timeout !== undefined) out += `timeout=${q.timeout}\n`;
    if (q.wrapuptime !== undefined) out += `wrapuptime=${q.wrapuptime}\n`;
    if (q.maxlen !== undefined) out += `maxlen=${q.maxlen}\n`;
    if (q.weight !== undefined) out += `weight=${q.weight}\n`;
    const members = q.members || [];
    for (const m of members) {
      out += `member=${m.interface}\n`;
    }
    out += '\n';
  }
  return out;
...
async function loadQueuesFromDisk() {
  try {
    const content = fs.readFileSync(QUEUES_CONF, 'utf8');
    queues = parseQueuesConf(content);
    console.log(`✅ Loaded ${queues.length} queues from queues.conf`);
  } catch (err) {
    console.warn('ℹ️  queues.conf not found or unreadable, starting fresh');
    queues = [];
  }
}

async function saveQueuesToDisk() {
  const payload = serializeQueuesConf(queues);
  fs.writeFileSync(QUEUES_CONF, payload, 'utf8');
  console.log(`✅ queues.conf updated with ${queues.length} queues`);
...
app.get('/api/asterisk/queues', authenticateAdmin, async (req, res) => {
  try {
    // Ensure queues are loaded from disk
...
// Create queue
app.post('/api/asterisk/queues', authenticateAdmin, async (req, res) => {
...
// Update queue
app.put('/api/asterisk/queues/:name', authenticateAdmin, async (req, res) => {
...
// Delete queue
app.delete('/api/asterisk/queues/:name', authenticateAdmin, async (req, res) => {
...
// Add member to queue
app.post('/api/asterisk/queues/:name/members', authenticateAdmin, async (req, res) => {
...
app.delete('/api/asterisk/queues/:name/members/:interface', authenticateAdmin, async (req, res) => {
  const queue = queues.find(q => q.name === req.params.name);
  if (!queue) {
    return res.status(404).json({ success: false, error: 'Queue not found' });
  }
